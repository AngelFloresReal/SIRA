...
@WebServlet (name = "removebuildingservlet", value = "/remove-building-servlet")
public class RemoveBuildingServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            String id =request.getParameter("id_building");
            ...
            try {
                if (!id.isEmpty()) {
                    PreparedStatement pst = connection.prepareStatement("SELECT * FROM edificios WHERE id_edificio = ?");
                    pst.setString(1, id);
                    ResultSet rs = pst.executeQuery();

                    if (rs.next()) { // Si la reserva existe
                        pst = connection.prepareStatement("DELETE FROM edificios WHERE id_edificio = ?");
                        pst.setString(1, id);
                        pst.executeUpdate();
