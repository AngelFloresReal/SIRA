...
@WebServlet (name = "editreservasservlet", value = "/edit-reservas-servlet")
public class EditReservasServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            System.out.println("ID DE RESERVA:" + idStr);
            ...
            if (idStr == null || titulo.isEmpty() || descripcion.isEmpty() || startStr.isEmpty() || endStr.isEmpty() || edificio == 0 || room == 0) {
                ...
                request.getRequestDispatcher("alert-reserva.jsp").forward(request, response);
            } else {
                ...
                if (start.isBefore(ahora) || start.getHour() < 7 || start.getHour() > 21 || start.getDayOfWeek().getValue() ==6 || start.getDayOfWeek().getValue() == 7 || end.getHour() < 7 || end.getHour() > 21 || end.getDayOfWeek().getValue() == 6 || end.getDayOfWeek().getValue() == 7){
                    ...
                    request.getRequestDispatcher("alert-reserva3.jsp").forward(request, response);
                } else {
                    ...
                    if (rs.next()) {
                        ...
                        request.getRequestDispatcher("alert-reserva4.jsp").forward(request, response);
                    } else {

                        int id = Integer.parseInt(idStr);
                        // Insert the new reservation
                        PreparedStatement ps = connection.prepareStatement("update reserva set tittle = ?, descripcion = ?, start = ?, end = ?,id_edificio = ?, id_aula = ? where id_reserva = " + id);
                        ...
                        RequestDispatcher rd = request.getRequestDispatcher("alert-edit-reserva1.jsp");
